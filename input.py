...
def indicator_gen(train, miss_indicator):
...
def noise_gen(data, miss_indicator, mean_arr):
    noise_data = np.array(data)
    for i in range(miss_indicator.shape[1]):
        miss_perm = np.where(miss_indicator[:, i] == 1)[0]
        noise_data[miss_perm, i] = mean_arr[i]
    return noise_data
